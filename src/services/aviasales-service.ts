...
export default class AviasalesService {
  private readonly API_URL = 'https://api.aviasales.ru/v1/search';

  async _getTickets(origin: string, destination: string, date: string): Promise<ITicket[]> {
    const response = await fetch(`${this.API_URL}?origin=${origin}&destination=${destination}&date=${date}`);
    const data = await response.json();
    return data.tickets;
  }

  async getTickets(origin: string, destination: string, date: string): Promise<ITicket[]> {
    return [
      {
        id: 1,
        price: 100,
  ...
  async _getTicketsByParams(params: { origin: string; destination: string; date: string }): Promise<ITicket[]> {
    const response = await fetch(
      `${this.API_URL}?origin=${params.origin}&destination=${params.destination}&date=${params.date}`
    );
    const data = await response.json();
    return data.tickets;
  }
...
// export default class AviasalesService {
//   private _apiBase = 'https://front-test.beta.aviasales.ru';
